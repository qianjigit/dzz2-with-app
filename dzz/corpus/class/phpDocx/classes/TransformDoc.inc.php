    ...
    public function generateXHTML()
    {
        ...
        foreach ($idImgs as $datsIdImgs) {
            $this->_xhtml = str_replace(
                ...
                "src=\"data/attachment/cache/files/files_" .
                md5($this->strFile)."/media/word/$pathImgs[$datsIdImgs]\"",
                $this->_xhtml
            );
        }
        ...
        if (!empty($pathZip)) {
            $package->extractTo(
                "data/attachment/cache/files/files_".md5($this->strFile)."/media", $pathZip
            );
            $package->close();
       }
