...
class table_corpus_organization_user extends dzz_table
{
	public function __construct() {

		$this->_table = 'corpus_organization_user';
		$this->_pk    = 'id';
			...
			if($uid!=getglobal('uid')){
				...
				if($uid!=getglobal('uid')){
					...
					$notevars=array(
									'from_id'=>$appid,
									'from_idtype'=>'app',
									'url'=>DZZSCRIPT.'?mod=corpus&op=org&orgid='.$orgid,
									'author'=>getglobal('username'),
									'authorid'=>getglobal('uid'),
									'dataline'=>dgmdate(TIMESTAMP),
									'name'=>getstr($org['name'],30),
									'permtitle'=>$permtitle[$perm],
									);
				
					$action='corpus_organization_user_add';
					$type='corpus_organization_user_add_'.$orgid;
	...
	public function insert_uids_by_orgid($orgid,$uids,$perm){
		$ouids=array();
		foreach(DB::fetch_all("select * from %t where orgid=%d and  uid in (%n)",array($this->_table,$orgid,$uids)) as $value){
		...
		foreach($uids as $uid){
		    ...
		    if(C::t('corpus_organization_user')->insert($userarr)){
				...
				if($uid!=getglobal('uid')){
					...
					$notevars=array(
									'from_id'=>$appid,
									'from_idtype'=>'app',
									'url'=>DZZSCRIPT.'?mod=corpus&op=org&orgid='.$corpus['orgid'],
									'author'=>getglobal('username'),
									'authorid'=>getglobal('uid'),
									'dataline'=>dgmdate(TIMESTAMP),
									'name'=>getstr($org['name'],30),
		...
		if($data['perm']>2 && DB::result_first("select COUNT(*) from %t where orgid=%d and perm>2",array($this->_table,$orgid))<2){
			...
			if($uid!=getglobal('uid')){
				...
				$notevars=array(
								'from_id'=>$appid,
								'from_idtype'=>'app',
								'url'=>DZZSCRIPT.'?mod=corpus&op=org&orgid='.$orgid.'#members',
								'author'=>getglobal('username'),
								'authorid'=>getglobal('uid'),
								'dataline'=>dgmdate(TIMESTAMP),
								'name'=>getstr($org['name'],30),
					...
					$action='corpus_organization_user_remove';
					$type='corpus_organization_user_remove_'.$orgid;
		...
		if($data['perm']!=$perm){//权限改变
			...
			if($uid!=getglobal('uid')){
				...
				$notevars=array(
					...
					$action='corpus_organization_user_change';
					$type='corpus_organization_user_change_'.$orgid;
